...
def main():
    ...
    # 按训练集类别比例分配总生成数量
    gen_alloc = old_scripts.diffusion.allocate_samples_by_ratio(
    ...
    for class_idx, class_name in enumerate(class_names):
        ...
        while remaining > 0:
            ...
            for i in range(generated_uint8.size(0)):
                pil_img = old_scripts.diffusion.uint8_tensor_to_pil(generated_uint8[i])
    ...
    # - 这里按“直接评估已生成目录”来调用。
    guided_eval_result = old_scripts.diffusion.evaluate_split_with_overall_and_per_class_metrics(
...
import old_scripts.diffusion as diffusion
from sklearn.metrics import (
    balanced_accuracy_score,
    confusion_matrix,
    classification_report,
