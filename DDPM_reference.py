...
class Unet(Module):
    def __init__(
        self,
        dim,  # 基础通道数（base channel width）
        init_dim=None,  # 输入卷积后的初始通道数；若为 None，则默认等于 dim
        out_dim=None,  # 最终输出通道数；若为 None，则用默认规则自动推断
        dim_mults=(1, 2, 4, 8),  # 每个分辨率阶段的通道倍率
        channels=3,  # 输入图像通道数，例如 RGB=3，灰度=1
        self_condition=False,  # 是否启用 self-conditioning（自条件）
        learned_variance=False,  # 是否学习方差；若是，输出通道数会翻倍
        learned_sinusoidal_cond=False,  # 是否使用可学习的正弦时间嵌入
        random_fourier_features=False,  # 是否使用随机 Fourier 特征时间嵌入
        learned_sinusoidal_dim=16,  # learned/random sinusoidal embedding 的维度
        sinusoidal_pos_emb_theta=10000,  # 标准正弦位置编码的 theta
        dropout=0.0,  # ResNet block 内的 dropout
        attn_dim_head=32,  # attention 每个 head 的维度
        attn_heads=4,  # attention 的 head 数
        full_attn=None,  # 哪些分辨率层使用 full attention；默认只有最深层用 full attention
        flash_attn=False,  # 是否启用 flash attention（取决于 Attend 的实现）
    ):
        super().__init__()
        ...
        # 如果启用 self-conditioning，就把 x_self_cond 和 x 在通道维拼接
        # 因而输入通道数要乘 2
        input_channels = channels * (2 if self_condition else 1)

        # 如果用户没传 init_dim，就默认等于 dim
        init_dim = default(init_dim, dim)

        # 创建初始 7x7 卷积：先把输入映射到 init_dim 通道
        # padding=3 可以让空间尺寸保持不变（7x7 卷积时）
        self.init_conv = nn.Conv2d(input_channels, init_dim, 7, padding=3)

        # dims 是每个分辨率阶段的通道数列表
        # 例如 dim=64, dim_mults=(1,2,4,8)
        # 则 dims = [init_dim, 64, 128, 256, 512]
        dims = [init_dim, *map(lambda m: dim * m, dim_mults)]

        # 把相邻阶段配成 (输入通道, 输出通道) 对
        # 例如 [(64,64), (64,128), (128,256), (256,512)]
        in_out = list(zip(dims[:-1], dims[1:]))

        # -----------------------------
        # 2) 构造时间步嵌入（time embedding）
        # -----------------------------

        # 时间嵌入最终会被 MLP 映射到 time_dim
        # 后面 ResnetBlock 会用它来生成 scale / shift 做条件调制
        time_dim = dim * 4

        # 是否使用 random/learned sinusoidal embedding
        self.random_or_learned_sinusoidal_cond = (
            learned_sinusoidal_cond or random_fourier_features
        )

        if self.random_or_learned_sinusoidal_cond:
            # 用 RandomOrLearnedSinusoidalPosEmb 处理时间
            # learned_sinusoidal_dim 是“频率向量”的维度
            sinu_pos_emb = RandomOrLearnedSinusoidalPosEmb(
                learned_sinusoidal_dim, random_fourier_features
            )
            # 这个模块输出维度 = learned_sinusoidal_dim + 1
            # 因为它会把原始 x 也拼进去
            fourier_dim = learned_sinusoidal_dim + 1
        else:
            # 用经典的 SinusoidalPosEmb
            sinu_pos_emb = SinusoidalPosEmb(dim, theta=sinusoidal_pos_emb_theta)
            fourier_dim = dim

        # 一个两层 MLP：把时间步编码成更高维的条件向量
        ...
        # 如果 full_attn 没指定，则默认只有最深层使用 full attention，
        # 其余层使用 linear attention
        if not full_attn:
        ...
        # cast_tuple 的作用：
        # 如果传入的是单个值，比如 attn_heads=4
        # 就把它扩展成长度为 num_stages 的元组，例如 (4,4,4,4)
        ...
        # 用 partial 先把 flash 参数固定住，生成一个“半配置好的 Attention 类”
        # partial 来自 Python 标准库 functools
        FullAttention = partial(Attention, flash=flash_attn)

        # 同理，先把 time_emb_dim 和 dropout 固定住
        # 后面调用 resnet_block(dim_in, dim_out) 就等价于
        ...
        for ind, (
            ...
            # 根据配置决定该层用 FullAttention 还是 LinearAttention
        ...
        # -----------------------------
        # 5) 中间瓶颈层（bottleneck / middle blocks）
        # -----------------------------
        ...
        # 6) 搭建上采样路径（decoder）
        # -----------------------------
        # 注意这里把 in_out、full_attn 等都 reversed 了
        # 因为 decoder 是按相反方向走回去的
        ...
        for ind, (
            ...
            # 这里输入通道为什么是 dim_out + dim_in？
            # 因为会和 skip connection 过来的特征做 concat
            self.ups.append(
                ModuleList(
                    [
        ...
        # 默认输出通道数：
        # - 如果不学习方差，则输出 channels
        # - 如果学习方差，则输出 2 * channels
        default_out_dim = channels * (1 if not learned_variance else 2)
        self.out_dim = default(out_dim, default_out_dim)

        # 最后还会把 decoder 输出和最开始的 r 再 concat 一次
        # 所以输入通道是 init_dim * 2
    ...
    def downsample_factor(self):
        # property 让你可以像访问属性一样写 model.downsample_factor
        # 这里表示总下采样倍数
        # 例如 len(self.downs)=4，则 factor = 2^(4-1)=8
        # 因为最后一个阶段不一定真的做 2x 下采样
    ...
    def forward(self, x, time, x_self_cond=None):
        ...
        # 如果启用了 self-conditioning，但没传 x_self_cond，
        # 就自动用全零张量代替
        ...
        # 保存一份最早期特征，最后还要拿来做一次全局 skip connection
        r = x.clone()

        # 把时间步编码成向量
        t = self.time_mlp(time)

        # h 用来保存 encoder 侧的中间特征，供 decoder 做 skip connection
        h = []
        ...
        for block1, block2, attn, downsample in self.downs:
            x = block1(x, t)
            h.append(x)  # 保存第一次 block 的输出

            x = block2(x, t)
            x = attn(x) + x  # attention 残差连接
            h.append(x)  # 保存第二次 block + attn 后的输出
        ...
        # 和最开始保存的浅层特征 r 做最后一次 skip connection
